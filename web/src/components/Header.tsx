...
const Header: React.FC<HeaderProps> = ({ logoSrc }) => {
    ...
    const isHomePage = window.location.pathname === '/' || window.location.pathname === '/onboarding' || window.location.pathname === '/onboarding/splynx';
    const shouldBlur = isHomePage ? isScrolled : true;

    return (
        <header className={`fixed top-0 left-0 right-0 z-50 ${shouldBlur ? 'bg-white/50 dark:bg-neutral-800/50 backdrop-blur-xs' : ''}`}>
